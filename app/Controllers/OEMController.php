...
class OEMController extends BaseController
{
    ...
    public function viewoemProfile($idd){
  	...
  	public function oemtagreqst(){

        $data= [];
       
        $data['oemreqst'] =  $this->loginModel->viewspecific('fastagrequest','*','requestedbytype',0);
      
         $cnt = sizeof($data['oemreqst']);
      
      for($i=0;$i< $cnt; $i++){
         $oemname[] = $this->loginModel->viewspecific('oem','*','oemid',$data['oemreqst'][$i]["requestedbyid"]);
      }
      
      $data['oemdata'] = $oemname;
      
        return view('admin/oemfastagrequest',$data);
    }
  	...
  	public function updaterequestStatus($idd,$val){
        ...
        $loginData = $this->loginModel->entry_update($table,$upclnm,$updtdata,$updtid);
            ...
            $this->session->setTempdata('success','OEM Request Status Updated Successfully', 3);
            return redirect()->to(base_url('secure/oemstockrequest'));
    }
  ...
  public function newcustomer()
    {
      ...
      if($cnt == 0){
        ...
        for($i=0;$i<$cnt;$i++){
            $fstgdetls[] = $this->salesagentModel->multiSrch('fasttag','*','barcode',$data['customer'][$i]["barcodeid"],'status',2);
        } 
      }
      
       $data['fstgdetls'] = $fstgdetls;
      
     // $data['customer'] = $this->oemModel->salesAgentnew($_SESSION['oemid'],1);
      
       return view('oem/newcustomerreport',$data);
    }
    ...
    public function existingcustomer()
     ...
     //   $data['customer'] = $this->oemModel->exstngidd($_SESSION['oemid'],1);
      
             

         return view('oem/existingcustomerreport',$data);
      }

...
class SalesManagerModel extends Model{
    ...
    public function searchh($table,$data,$whr1,$whr2){
  	...
  	public function fastaglike($data){

        $builder = $this->db->table('fasttag');
        $builder->select('*');
        //$builder->where("(barcode LIKE '%".$data."%')", NULL, FALSE);
        $builder->like('barcode', $data);
        $result = $builder->get();
        return $result->getResultArray();
    ...
    public function vehiclenumberlike($table,$data){

        $builder = $this->db->table($table);
        $builder->select('*');
        //$builder->where("(barcode LIKE '%".$data."%')", NULL, FALSE);
        $builder->like('vehiclechasisnumber', $data);
        $result = $builder->get();
        return $result->getResultArray();

    }
  
  	public function barcodeadvsrch($frstnum,$lastnum){

        $builder = $this->db->table('fasttag');
        $builder->select('*');
        $builder->where("barcode BETWEEN '".$frstnum."' AND '".$lastnum."'");
        $result = $builder->get();
        return $result->getResultArray();
  	...
  	public function barcdeSrchIndv($data,$id,$typ){

        $builder = $this->db->table('fasttag');
        $builder->select('fasttag.fasttagid,fastaginventory.fasttagid,fasttag.barcode,fasttag.tagid,fasttag.tid,fasttag.classoftag,fastaginventory.allotedto,fastaginventory.allotedtotype,fastaginventory.status,fastaginventory.inventoryid');
        $builder->join('fastaginventory', 'fastaginventory.fasttagid = fasttag.fasttagid');
        $builder->where('fastaginventory.fasttagid', $data);
        $builder->where('allotedto',$id);
        $builder->where('allotedtotype',$typ);
        $result = $builder->get();
        return $result->getResultArray();
  	...
  	public function fastaglikeManager($data,$id,$typ){

        $builder = $this->db->table('fasttag');
        $builder->select('fasttag.fasttagid,fastaginventory.fasttagid,fasttag.barcode,fasttag.tagid,fasttag.tid,fasttag.classoftag,fastaginventory.allotedto,fastaginventory.allotedtotype,fastaginventory.status,fastaginventory.inventoryid');
        $builder->join('fastaginventory', 'fastaginventory.fasttagid = fasttag.fasttagid');
        $builder->like('fasttag.barcode', $data);
        $builder->where('allotedto',$id);
        $builder->where('allotedtotype',$typ);
        $result = $builder->get();
        return $result->getResultArray();
  	...
  	public function deleteentry($table,$clmnm,$id){

        $builder = $this->db->table($table);
        $builder->where($clmnm, $id);
        $builder->delete();
  	...
  	public function chkduplcate($table,$where){
  	...
  	public function getLastFastag($tagid){

        $builder = $this->db->table('fastaginventory');
        $builder->select('*');
        $builder->where('fasttagid',$tagid);
        $builder->orderBy('inventoryid ', 'desc');
        $builder->limit(1);
        $result = $builder->get();
        return $result->getResultArray();
